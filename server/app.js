...
export function createApp({
  ...
  const worker = createTaskWorker({ store, secrets, fetchImpl, logger, promptCipher });
  ...
  app.get('/api/health/upstream', requireAuth, (req, res) => {
    res.json({ ok: true, configured: secrets.has(req.auth.session.hash) });
  });
  ...
  app.post('/api/auth/logout', requireAuth, requireCsrf, route(async (req, res) => {
    secrets.delete(req.auth.session.hash);
  ...
  app.get('/api/config', requireAuth, (req, res) => {
    const config = secrets.get(req.auth.session.hash);
  ...
  app.post('/api/config/discover', requireAuth, requireChangedPassword, requireCsrf,
    ...
    route(async (req, res) => {
      const previous = secrets.get(req.auth.session.hash);
      ...
      const result = await upstreamJson(fetchImpl, endpoints.modelsUrl, { method: 'GET', apiKey, logger });
      ...
      secrets.set(req.auth.session.hash, config);
      await store.rebindAuthTasks(req.auth.user.id, req.auth.session.hash, config.keyFingerprint);
      worker.tick();
      res.json({ ok: true, config: publicConfig(config), models: models.map(modelDescriptor), durationMs: result.durationMs });
    }));

  app.put('/api/config/model', requireAuth, requireChangedPassword, requireCsrf, route(async (req, res) => {
    const config = requireSecret(req, secrets);
  ...
  app.post('/api/video/tasks', requireAuth, requireChangedPassword, requireCsrf,
    ...
    route(async (req, res) => {
      const config = requireSecret(req, secrets);
      const model = String(req.body.model || config.model);
      if (!config.models.includes(model)) throw publicError(400, '所选模型不在当前 Key 的可用列表中');
      const validated = validation(() => validateModelParameters(model, req.body.resolution, req.body.duration));
      const payload = validation(() => buildVideoPayload({ ...req.body, model, resolution: validated.resolution, duration: validated.duration }));
      ...
      const task = {
        ...
        pricingSnapshot: pricing, usage: null, cost: null, videoUrl: null,
        createdAt: now, updatedAt: now
      };
      await store.createTask(task);
      worker.tick();
      res.status(201).json({ task: publicTask(task, promptCipher), queuePosition: globalQueued + 1 });
    }));
  ...
  app.get('/api/video/tasks/:taskId', requireAuth, requireChangedPassword, route(async (req, res) => {
    ...
    if (!task || task.userId !== req.auth.user.id) throw publicError(404, '没有找到这个任务');
    worker.tick();
  ...
  app.post('/api/admin/users', requireAuth, requireChangedPassword, requireAdmin, requireCsrf, route(async (req, res) => {
    ...
    let user;
    try { user = await store.createUser({ id: crypto.randomUUID(), username, passwordHash, role: req.body.role === 'ADMIN' ? 'ADMIN' : 'USER', mustChangePassword: true }); }
...
function requireSecret(req, secrets) {
  const config = secrets.get(req.auth.session.hash);
...
function publicTask(task, cipher) {
  ...
  result.pricing = task.pricingSnapshot && { inputRate: task.pricingSnapshot.inputRate, outputRate: task.pricingSnapshot.outputRate, currency: task.pricingSnapshot.currency, version: task.pricingSnapshot.version };
  return result;
}
function adminTask(task) {
  return {
    id: task.id, userId: task.userId, remoteTaskId: task.remoteTaskId, status: task.status,
    model: task.model, resolution: task.resolution, usage: task.usage, cost: task.cost,
    code: task.code, message: task.message, createdAt: task.createdAt, completedAt: task.completedAt
  };
}
...
function sendError(res, error, logger) {
  ...
  res.status(status).json({ error: { message: error.publicMessage || (status < 500 ? error.message : '服务暂时不可用，请稍后重试'), code: error.code, upstreamStatus: error.upstreamStatus, requestId: error.requestId } });
}
